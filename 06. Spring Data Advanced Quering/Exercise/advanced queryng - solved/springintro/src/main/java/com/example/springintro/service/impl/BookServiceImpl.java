...
public class BookServiceImpl implements BookService {
    ...
    public List<String> findAllBookTitlesWithPriceLessThan5MoreThan40() {
        return bookRepository
                .findAllByPriceLessThanOrPriceGreaterThan(BigDecimal.valueOf(5L), BigDecimal.valueOf(40))
                .stream()
    ...
    @Override
    public List<String> findAllBooTitlesWhereTitleContainString(String containString) {
        return bookRepository
                .findAllByTitleContains(containString)
                .stream()
                .map(Book::getTitle)
                .collect(Collectors.toList());
    }
    ...
    @Override
    public List<String> findAllTitleWithAuthorWithLastNameStartsWith(String startStringInput) {
        return bookRepository
                .findAllByAuthor_LastNameStartsWith(startStringInput)
                .stream()
    ...
    @Override
    public List<String> findInformationAboutBook(String bookInput) {
        return bookRepository
                .findBookByTitle(bookInput)
                .stream()
                .map(book -> String.format("%s %s %s %.2f",book.getTitle(),book.getEditionType().name(),book.getAgeRestriction().name(),book.getPrice()))
                .collect(Collectors.toList());
    }
